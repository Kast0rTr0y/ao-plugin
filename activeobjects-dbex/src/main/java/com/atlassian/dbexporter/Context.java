...
public final class Context
{
    ...
    /**
     * Gets the single element of the given type in the import context throwing an exception if none is present.
     *
     * @param type the type of the element to look for
     * @param <T> the type parameter of the type to look for
     * @return the element if it exists, {@code null} otherwise
     * @throws com.atlassian.dbexporter.DbImportException if no element of type {@code T} is found.
     * @throws IllegalStateException if more than one element of the given type are present in the context. If you need
     * to get multiple elements of type {@code T} use the {@link #getAll(Class)} method.
     * @see #get(Class)
     * @see #getAll(Class)
    ...
    {
        final T instance = get(type);
        if (instance == null)
        {
            throw new DbImportException("Could not find required element of type " + type.getName() + " in import context");
        }
        return instance;
    }
     ...
     * @return {@code this}
     */
    public Context putAll(Collection<? extends Object> objs)
    {
        context.addAll(objs);
        return this;
    }
